...
public class Advice {
    public record  Response<T>(String code, @Nullable T data, @Nullable String message) {
        ...
        public static Response<?> of(RuntimeException exception) {
            return new Response<>(BackendException.ErrorCode.INTERNAL_SERVER_ERROR.getCode(), null, exception.getMessage());
        }
    }

    @ExceptionHandler
    public Response<?> handleValidationException(MethodArgumentNotValidException exception) {
        return new Response<>(BackendException.ErrorCode.BAD_REQUEST.getCode(), null, exception.getMessage());
    }
}
